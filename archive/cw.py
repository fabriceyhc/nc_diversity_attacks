...
class L2Adversary(object):
    ...
    def __call__(self, model, inputs, targets, to_numpy=True):
        ...
        targets_oh.scatter_(1, targets.unsqueeze(1).long(), 1.0) # FHC: previously targets_oh.scatter_(1, targets.unsqueeze(1), 1.0) 
        ...
        for sstep in range(self.binary_search_steps):
            ...
            print('Step', sstep, 'Using scale consts:', list(scale_consts_np))  # FIXME
    ...
    def _optimize(self, model, optimizer, inputs_tanh_var, pert_tanh_var,
        ...
        # the perturbed activation before softmax # FHC: how do we know that it's before softmax??
        ...
        # FHC: Out of the 7 functions in the paper, the one implemented below is f_2 (original one used for L2-norm attacks)
        # Alternative: $\sum_{i=1}^N LOSS[f(x_i+\delta), y_i] + \lambda \cdot DIVERGENCE[f(x_i+\delta),DISTRIBUTION_{uniform}]$
        ...
        batch_loss = batch_loss_var.data # FHC: previously batch_loss_var.data[0]   # type: float
    ...
    def _compensate_confidence(self, outputs, targets):
        ...
        targets = targets.astype(int) # FHC: added
        if self.targeted:
            # for each image $i$:
